...
mppi_dir = os.path.dirname(os.path.abspath(__file__))

# Add it to the Python path
sys.path.insert(0, mppi_dir)
# (Added to fix an weeor whe importing MPPI_Utils)
...
class MPPIPlanner(ABC):
    ...
    def __init__(self, cfg: Union[MPPIConfig, dict], nx: int, dynamics: Callable, running_cost: Callable, prior: Optional[Callable] = None):
        ...
        self.return_states = cfg.return_states
    ...
    def _compute_rollout_costs_once(self, perturbed_actions):
        """
        ...
        for t in range(T):
            ...
            # Last rollout is a braking maneuver
            if self.sample_null_action:
                u[self.K - 1, :] = torch.zeros_like(u[self.K - 1, :])
                self.perturbed_action[self.K - 1][t] = u[self.K - 1, :]
        ...
        c = self._running_cost(states)
        c = c.reshape(T, K)

        ## SUm along the time axis to get cost of each sample totoal
    ...
    def _compute_total_cost_batch_simple(self):
        ...
        if self.compute_cost_once:
            self.cost_total, self.states, self.actions = self._compute_rollout_costs_once(self.perturbed_action)
        else:
